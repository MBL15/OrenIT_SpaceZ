...
/** classId -> AssignmentOut[] для моков teacher/admin assignments */
const mockAssignmentsByClass = Object.create(null)
let mockNextAssignmentId = 1
...
export function devMockApiFetch(path, init = {}) {
  ...
  if (url === '/admin/classes' && method === 'GET') {
    return json(
      mockClassStore.classes.map((c) => ({
        id: c.id,
        name: c.name,
        teacher_id: c.teacher_id ?? 2,
        teacher_login: 'teacher_demo',
        teacher_display_name: 'Демо-учитель',
      })),
    )
  }

  const adminAssignments = url.match(/^\/admin\/classes\/(\d+)\/assignments$/)
  if (adminAssignments && method === 'GET') {
    const classId = Number(adminAssignments[1])
    return json(mockAssignmentsByClass[classId] || [])
  }
  ...
  if (url === '/teacher/classes' && method === 'POST') {
    ...
    const c = {
      id: mockClassStore.nextId++,
      name: body.name || 'Класс',
      teacher_id: 2,
    }
  ...
  const teacherAssignments = url.match(/^\/teacher\/classes\/(\d+)\/assignments$/)
  if (teacherAssignments) {
    const classId = Number(teacherAssignments[1])
    if (method === 'GET') return json(mockAssignmentsByClass[classId] || [])
    if (method === 'POST') {
      let body = {}
      try {
        body = JSON.parse(init.body || '{}')
      ...
      const id = mockNextAssignmentId++
      const row = {
        id,
        class_id: classId,
        task_template_id: body.task_template_id ?? 101,
        lesson_id: 1,
        lesson_title: 'Асгард',
        task_title: 'Практика: старт',
        note: body.note ?? null,
        created_at: new Date().toISOString().slice(0, 19).replace('T', ' '),
        reward_coins: body.reward_coins ?? 0,
        reward_xp: body.reward_xp ?? 0,
      }
      if (!mockAssignmentsByClass[classId]) mockAssignmentsByClass[classId] = []
      mockAssignmentsByClass[classId].push(row)
      return json(row)
    }
  }
  const delAsg = url.match(/^\/teacher\/classes\/(\d+)\/assignments\/(\d+)$/)
  if (delAsg && method === 'DELETE') {
    const classId = Number(delAsg[1])
    const aid = Number(delAsg[2])
    const list = mockAssignmentsByClass[classId]
    if (list) {
      const i = list.findIndex((x) => x.id === aid)
